...
console.log('🚀 Switchgrid R65 INJECTION Function Ready');
...
function parseR65InjectionData(raw: any) {
  console.log('🔧 Parsing R65 INJECTION data...');
  ...
  // Pour INJECTION, on cherche la grandeur d'injection/production
  const injectionGrandeur = grandeurs.find((g: any) => {
    const metier = (g.grandeurMetier || '').toUpperCase();
    const physique = (g.grandeurPhysique || '').toUpperCase();
    return (metier.includes('PROD') || metier.includes('INJECTION')) &&
           (physique === 'EA' || physique === 'E' || physique === 'EAI');
  });

  if (!injectionGrandeur) {
    console.error('❌ No injection grandeur found');
  ...
  console.log(`✅ Found grandeur: ${injectionGrandeur.grandeurMetier}`);

  const points = injectionGrandeur.points || [];
  const unit = (injectionGrandeur.unite || '').toLowerCase();
  ...
  const rows = points.map((p: any) => {
    ...
    return {
      ...
      energy_by_cadran: { INJECTION: energyKwh }
    };
  });
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'create_r65_injection_order_and_poll') {
      if (!prm || !consent_id) {
      ...
      console.log(`📅 Creating R65 INJECTION order: ${sinceDate} → ${untilDate}`);
      ...
      if (returnRows) {
        ...
        const rows = parseR65InjectionData(rawData);
    ...
    return new Response(JSON.stringify({
      error: 'Unknown action. Use: create_r65_injection_order_and_poll or fetch_data_url'
    }), {
      status: 400,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    });
