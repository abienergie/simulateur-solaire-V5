...
console.log('🚀 Switchgrid R66 Function Ready');
...
function parseR66Data(raw: any) {
  console.log('🔧 Parsing R66 data...');
  ...
  // Pour R66, on cherche la puissance maximale
  // Essayons plusieurs patterns pour être plus flexible
  const powerGrandeur = grandeurs.find((g: any) => {
    ...
    // Patterns possibles pour puissance max
    return (
      metier.includes('PMAX') ||
  ...
  const rows = points.map((p: any) => {
    ...
    // Force conversion en kW (diviser par 1000 car les valeurs sont en W)
    const powerKw = Math.round((value / 1000) * 1000) / 1000;

    return {
      date,
      power_max_kw: powerKw,
      time_of_max: p.h || p.time || null
    };
  });
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'create_r66_order_and_poll') {
      if (!prm || !consent_id) {
      ...
      console.log(`📅 Creating R66 order: ${sinceDate} → ${untilDate}`);
      ...
      const request = await pollOrder(order.id, 'R66_SYNC');
      ...
      if (returnRows) {
        ...
        const rows = parseR66Data(rawData);
    ...
    return new Response(JSON.stringify({
      error: 'Unknown action. Use: create_r66_order_and_poll or fetch_data_url'
    }), {
      status: 400,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    });
